...
int main() {
   ...
   // snprintf(data.system_api_key, sizeof(data.system_api_key), "SUPER_SECRET_API_KEY_12345");
    snprintf(data.system_api_key, sizeof(data.system_api_key), "tdausecret123");
    ...
    for (int i = 0; data.username[i]; i++) { // Remove automatic '\n' in fgets()
        if (data.username[i] == '\n') {
            mark = 1;
            data.username[i] = '\0';
        }
    ...
    if (mark == 0) { // Flush excess in stdin
        while (((temp = getchar()) != '\n') && (temp != EOF)) {}
    }
    ...
    mark = 0;
    for (int i = 0; data.password[i]; i++) { // Remove automatic '\n' from fgets()
        if (data.password[i] == '\n') {
            mark = 1;
            data.password[i] = '\0';
        }
